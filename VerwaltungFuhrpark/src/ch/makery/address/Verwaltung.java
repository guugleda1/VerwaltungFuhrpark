...
public class Verwaltung {
	...
	public ObservableList<Fahrzeug> testFahrzeugData(Mitarbeiter mitarbeiter){	//gibt liste aller für mitarbeiter buchbaren fahrzeuge zurück
		ObservableList<Fahrzeug> a = FXCollections.observableArrayList();
		for(int i = 0; i < getFahrzeugData().size(); i++){
			if((mitarbeiter.getDriverslicense().contains(getFahrzeugData().get(i).getEssentialDriverslicense()))&&getFahrzeugData().get(i).getAvailability()){
	...
	public ObservableList<Mitarbeiter> testMitarbeiterData(Fahrzeug fahrzeug){	//gibt liste aller mitarbeiter zurück, die fahrzeug fahren könnten
		ObservableList<Mitarbeiter> a = FXCollections.observableArrayList();
		for(int i = 0; i < getPersonData().size(); i++){
			if((getPersonData().get(i).getDriverslicense().contains(fahrzeug.getEssentialDriverslicense()))&&!(getPersonData().get(i).getHatFahrzeug())){
	...
	private ObservableList<Mitarbeiter> makeObservableListMitarbeiter(ArrayList<Mitarbeiter> b){	//wandelt arraylist in observablelist um
		ObservableList<Mitarbeiter> a = FXCollections.observableArrayList();
	...
	private ArrayList<Mitarbeiter> makeArrayListMitarbeiter(ObservableList<Mitarbeiter> l){	//wandelt observablelist in arraylist um
	...
	public void speichern(){	//wird bei programmende aufgerufen um xml-dateien zu überschreiben
